...
public class ItemTurtle extends Item {
    public ItemTurtle(World world, TextureAtlas textureAtlas, Vector2 position) {
        super(world, textureAtlas, position);
        setRegion(textureAtlas.findRegion("turtle"),0,0,16,24);
        setBounds(getX(), getY(), Utils.convertPixelsToMeters(16f), Utils.convertPixelsToMeters(16f));
        Random r = new Random();
        float velX = r.nextBoolean() ? 0.7f : -0.7f;
